...
def filter_df(df, threshold, sample, replicate):
    ...
    out_dir = create_dir(out_downsample_dir, f"04_01_04_filtered_abundance_{threshold}")
    df.to_csv(os.path.join(out_dir, f"{replicate}_{sample}.csv"), index=False)

    return df


def concat_df(df1, df2, threshold, bin):
    """
    Concatenate dataframes for each replicate.
    :param df1:
    :param df2:
    :param threshold:
    :param bin:
    ...
    logger.info(f"Concatenating dataframes for {bin} ...")
    # Concatenate df1 and df2
    merged_df = pd.concat([df1, df2], ignore_index=True)
    merged_df = merged_df.groupby("AA_CHANGE").sum().reset_index()

    out_dir = create_dir(out_downsample_dir, f"04_01_04_filtered_abundance_{threshold}")
    merged_df.to_csv(os.path.join(out_dir, f"{bin}_concat.csv"), index=False)
...
def binned_distribution(file, bin):
    ...
    out_dir = create_dir(root_dir, "04_01_dSortSeq_inputs")
    out_downsample_dir = create_dir(out_dir, f"downsample_{downsample_num}")

    if bin == "G":
        # Export merged_df to csv with header and without index
        merged_df.to_csv(os.path.join(out_downsample_dir, f"dark_binned_distribution.csv"), index=False)

    elif bin == "L":
        # Export merged_df to csv with header and without index
        merged_df.to_csv(os.path.join(out_downsample_dir, f"light_binned_distribution.csv"), index=False)

    elif bin == "Y":
        # Export merged_df to csv with header and without index
        merged_df.to_csv(os.path.join(out_downsample_dir, f"ymScarlet_binned_distribution.csv"), index=False)
...
def mixing_coefficient(file, bin):
    ...
    out_downsample_dir = os.path.join(root_dir, "04_01_dSortSeq_inputs", f"downsample_{downsample_num}")

    if bin == "G":
        # Export merged_df to csv without header and without index
        merged_df.to_csv(os.path.join(out_downsample_dir, f"dark_mixing_coefficient.csv"), header=False,
                         index=False)
    elif bin == "L":
        # Export merged_df to csv without header and without index
        merged_df.to_csv(os.path.join(out_downsample_dir, f"light_mixing_coefficient.csv"), header=False,
                         index=False)
    elif bin == "Y":
        # Export merged_df to csv without header and without index
        merged_df.to_csv(os.path.join(out_downsample_dir, f"ymScarlet_mixing_coefficient.csv"), header=False,
                         index=False)
...
if __name__ == '__main__':
    ...
    # Set vcf root directory.
    vcf_root_dir = os.path.join(root_dir, "03_01_vcf_files")
    ...
    parser.add_argument("-a", "--abundance", help="Abundance threshold", type=int, default=15)
    args = parser.parse_args()
    bq_threshold = args.bq
    downsample_num = args.number
    threshold = args.abundance
    downsample_dir = os.path.join(vcf_root_dir, f"downsample_{downsample_num}")
    logger.info(f"Downsample directory: {downsample_dir}")

    # Set output directory.
    out_root = create_dir(root_dir, "04_01_variant_analysis")
    out_downsample_dir = create_dir(out_root, f"downsample_{downsample_num}")
    downsample_none_dir = os.path.join(out_root, "downsample_none")
    ...
    if not os.path.isdir(downsample_none_dir):
        ...
        downsample_none_files = get_undownsample_files("*.vcf", vcf_root_dir)
        vcf_out_dir = None
        for file in downsample_none_files:
            vcf_out_dir = create_dir(downsample_none_dir, "04_01_01_raw_csv")
            vcf_to_csv(file, vcf_out_dir)

        raw_csv_files = get_files(os.path.join(vcf_out_dir, "R*.csv"))
        for file in raw_csv_files:
            bq_filter_out_dir = create_dir(downsample_none_dir, f"04_01_02_filtered_bq_{bq_threshold}")
            bq_filter(file, bq_threshold, bq_filter_out_dir)

        bq_filtered_files = get_files(os.path.join(downsample_none_dir, f"04_01_02_filtered_bq_{bq_threshold}", "R*.parquet"))
        for file in bq_filtered_files:
            all_variants_out_dir = create_dir(downsample_none_dir, "04_01_03_single_double_variants")
            get_single_double_variants(file, all_variants_out_dir)



    # Get vcf files.
    vcf_files = get_files(os.path.join(downsample_dir, "*.vcf"))

    for file in vcf_files:
        # Convert vcf to csv.
        vcf_out_dir = create_dir(out_downsample_dir, "04_01_01_raw_csv")
        vcf_to_csv(file, vcf_out_dir)

    # Get raw csv files.
    raw_csv_files = get_files(os.path.join(out_downsample_dir, "04_01_01_raw_csv", "*.csv"))

    for file in raw_csv_files:
        # Extract BQ values from VCF files and save to parquet file.
        bq_filter_out_dir = create_dir(out_downsample_dir, f"04_01_02_filtered_bq_{bq_threshold}")
        bq_filter(file, bq_threshold, bq_filter_out_dir)

    # Get all bq filtered files
    bq_filtered_files = get_files(os.path.join(out_downsample_dir, f"04_01_02_filtered_bq_{bq_threshold}", "*.parquet"))

    for file in bq_filtered_files:
        all_variants_out_dir = create_dir(out_downsample_dir, "04_01_03_single_double_variants")
        get_single_double_variants(file, all_variants_out_dir)


    # Get all single double variants files
    single_double_variants_files = (get_files(os.path.join(out_downsample_dir, "04_01_03_single_double_variants", "*.parquet")) +
    ...
    for bin in bins:
        ...
        concat_df(R1_filtered_df, R2_filtered_df, threshold, bin)

        del R1_merged_df, R2_merged_df

    # Generate input files for 05_dSortSeq_FP64.py
    abundance_filtered_files = get_files(os.path.join(out_downsample_dir, f"04_01_04_filtered_abundance_{threshold}", "*concat.csv"))

    for file in abundance_filtered_files:
        bin = file.split("/")[-1].split("_")[0]
        logger.info("Generating input files for 05_dSortSeq_FP64.py...")
